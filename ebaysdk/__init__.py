...
def get_version():
    ...
    version = re.search(r"^__version__ = ['\"]([^'\"]*)['\"]",
        open(VERSIONFILE, "r").read(), re.M).group(1)
...
class ebaybase(object):
    ...
    def _execute_http_request(self):
       ...
       # print request_url
       # print request_xml
        result = urlfetch.fetch(url="http://svcs.ebay.com/services/search/FindingService/v1",
                        payload=request_xml,
                        method=myMethod,
                        headers=headerdict)
        #print result;
        return result.content;
    def _execute_http_request_old(self):
